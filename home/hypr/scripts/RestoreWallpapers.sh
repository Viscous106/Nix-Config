...
# Initialize awww-daemon if not running (awww-daemon is a makeWrapper stub on
# NixOS, so its live comm isn't "awww-daemon" — use `awww query` to detect a
# live daemon instead of an exact-name pgrep)
